...
Usage:
Make sure this file is somewhere the cogserver can find it:
...
# Convenience class to create a cogserver command in a format consistent with the other commands
...
class Bio:
    ...
    def load_scheme(self):
        """
        Load initial scheme files in scheme_files list into the atomspace.
    ...
    def get_genesets_for_gene(self, gene):
        if gene in self.member_sets_dict:
            ...
            genesets = self.member_sets_dict[gene] = scheme_eval_list(
                self.a, '(get_sets_for_member "' + gene.name + '")')
        if V:
